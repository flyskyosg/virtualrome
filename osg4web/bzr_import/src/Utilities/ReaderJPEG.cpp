...
unsigned char * Utilities::Reader::JPEG::LoadImageFromFile(std::string filename, int& width, int& height, int& numComponents)
{
	std::ifstream istream(filename.c_str(), std::ios::in | std::ios::binary);
	if(!istream) 
		return NULL;
	
	return simage_jpeg_load(istream, &width, &height, &numComponents);
}